...
import Taro, { getApp,getSystemInfoSync } from '@tarojs/taro';
console.log(Taro,"pppp")
const SYSTEMINFO = getSystemInfoSync();


let networkType;
const webpSupport = SYSTEMINFO?.platform === 'android';  // 是否支持webp
 ...
 */
export const parseImgUrl = (url: string, width?: number, height?: number, cut?: string) => {
    if (!url) return '';
    ...
    // 去除图片后缀的多余字符
    const rUrl = url.match(/(\S*(jpg|jpeg|png|webp|gif|JPG|JPEG|PNG|WEBP|GIF))\s*/g);
    if (!rUrl) {
        return url;
    }
    ...
    // 非JFS 或CDN 图片不处理
    if (!isJfsImg || !/(m|img\d{1,2})\.360buyimg\.com/.test(url) || !/\.(jpg|jpeg|png|webp)/.test(url)) {
        return url;
    }
